...
def init_events(app):
    if app.config.get("CACHE_TYPE") in ["redis", "rediss", "redis-cluster", "RedisCache", "RedisSentinelCache", "RedisClusterCache"]:
        app.events_manager = RedisEventManager()
        print("Using RedisEventManager for event handling")
